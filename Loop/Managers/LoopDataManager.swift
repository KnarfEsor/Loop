...
final class LoopDataManager {
    ...
    func addGlucose(
        ...
        glucoseStore.addGlucoseValues(values, device: device) { (success, values, error) in
            if success {
                self.dataAccessQueue.async {
                    self.glucoseUpdated = true // new glucose received, allow integral RC update
    ...
    struct retrospectiveCorrection {
        ...
        func updateRetrospectiveCorrection(discrepancy: Double,
            ...
            if (retrospectiveCorrection.previousDiscrepancy * discrepancy < 0 ||
                ...
                NSLog("myLoop: integral RC: reset integral action")
                ...
                if(glucoseUpdated){
                    NSLog("myLoop: integral RC: update integral action")
                    ...
                    NSLog("myLoop: integral RC: glucose not updated, keep integral action")
                }
            }
